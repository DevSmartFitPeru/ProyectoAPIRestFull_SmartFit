...
def accesos(plan,fecha_inicio,fecha_fin):
    try:
        cursor = connect(aws_access_key_id="AKIA4LTBLLTUCHTCM2ZY", aws_secret_access_key="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", s3_staging_dir="s3://7-smartfit-da-de-lake-artifacts-athena-latam/", region_name="us-east-1", work_group="peru", schema_name="prod_lake_modeled_refined").cursor()
        cursor.execute("select person_id CODIGO_CLIENTE ,purchase_id ID_COMPRA ,name NOMBRES ,cached_status ESTADO_CLIENTE ,acronym CODIGO_UNIDAD ,date_format(cancelled,'%Y-%m-%d')FECHA_CANCELAMIENTO ,date_format(purchase_date ,'%Y-%m-%d')FECHA_COMPRA ,plan PLAN_CLIENTE ,document NRO_DOCUMENTO ,date_format(due_at ,'%Y-%m-%d') DUE_AT  ,access_acronym  ACCESO_ACRONYM ,date_format(date,'%Y-%m-%d') FECHA ,date_format(NOW(),'%Y-%m-%d') FECHA_PROCESAMIENTO  prod_lake_refined_modeled_latam_lgpd.acessos_detalhados where lower(pais) ='peru'and plan = '"+str(plan)+"' and date_format (date,'%Y-%m-%d') between '"+str(fecha_inicio)+"' and '"+str(fecha_fin)+"'")
        resultado = []
        for row in cursor:
            content = {
