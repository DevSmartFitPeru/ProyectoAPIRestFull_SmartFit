...
def ingenico(fecha_inicio,fecha_fin,geografico):
        try:
            cars = []
            cursor = conn.cursor()
            cursor.execute(" SELECT ING.COMERCIO_ORDEN_ID ,CONVERT(DATE,ING.FECHA,103)FECHA ,ING.UNIDAD ,ING.VALOR_PAGO ,ING.ESTADO ,ING.TIPO_COBRANZA ,ING.TENTATIVA_DE_COBRANZA_TOTAL ,ING.CODIGO_PAGAMENTO ,ING.COD_ALUMNO ,ING.MODO_INGRESO ,ING.TIPO ,ING.COMERCIO ,ING.COD_PRODUCTO ,ING.PAN ,ING.COD_ACCION ,ING.MOTIVO_NEGACION ,ING.RESP_COD_MOP ,ING.RESP_MOP ,ING.RESP_COD_PSP ,ING.RESP_PSP ,ING.RESP_EXT_PSP ,ING.BIN ,ING.PLANN ,TRY_CONVERT(DATE,ING.FECHA_VENCIMIENTO,103)FECHA_VENCIMIENTO ,TRY_CONVERT(DATE,ING.FECHA_PUBLICACION,103)FECHA_PUBLICACION ,ING.NRO_COMPROBANTE ,ING.TIPO_COMPROBANTE FROM VOXIVA.DWH.INGENICO ING LEFT JOIN (SELECT  * FROM PROCESOS_SMARTFIT.SMARTFIT.UNIDADES_SMARTFIT_PERU WHERE ESTADO_UNIDAD = 'ACTIVO' AND RUC IN ('20601464625','20511776491','20600597940')) LOC ON ING.UNIDAD = LOC.CODIGO AND CONVERT(DATE,FECHA,103) BETWEEN '"+str(fecha_inicio)+"' and '"+str(fecha_fin)+"' AND LOC.DEPARTAMENTO = '"+str(geografico)+"' ")
            for row in cursor.fetchall():
                content = {
