...
def location():
    try:
        cursor = connect(aws_access_key_id="AKIA4LTBLLTULQVURH6J", aws_secret_access_key="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", s3_staging_dir="s3://7-smartfit-da-de-lake-artifacts-athena-oic-peru", region_name="us-east-1", work_group="peru", schema_name="prod_lake_modeled_refined").cursor()
...
def transaction_latam(fecha_inicio,fecha_fin):
    try:
        cursor = connect(aws_access_key_id="AKIA4LTBLLTULQVURH6J", aws_secret_access_key="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", s3_staging_dir="s3://7-smartfit-da-de-lake-artifacts-athena-oic-peru", region_name="us-east-1", work_group="oic_peru", schema_name="prod_lake_modeled_refined").cursor()
...
def payment_ar(fecha_inicio,fecha_fin):
    try:
        cursor = connect(aws_access_key_id="AKIA4LTBLLTULQVURH6J",
                         aws_secret_access_key="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",
                         s3_staging_dir="s3://7-smartfit-da-de-lake-artifacts-athena-oic-peru", region_name="us-east-1",
                         work_group="oic_peru", schema_name="prod_lake_ss_refined").cursor()
...
def receivable_otc(fecha_inicio,fecha_fin):
    try:
        ...
        cursor = connect(aws_access_key_id="AKIA4LTBLLTULQVURH6J",
                         ...
                         s3_staging_dir="s3://7-smartfit-da-de-lake-artifacts-athena-oic-peru", region_name="us-east-1",
                         work_group="oic_peru", schema_name="prod_lake_modeled_refined").cursor()
        cursor.execute("select distinct otc.minifactu_id ,otc.country,otc.operation,rece.registration_gym_student ,otc.front_id ,otc.origin_system ,CASE WHEN rece.erp_clustered_receivable_id is null THEN 0 ELSE rece.erp_clustered_receivable_id end erp_clustered_receivable_id,CASE WHEN sum(item.list_price) is null THEN 0 ELSE sum(item.list_price) end list_price,CASE WHEN sum(item.sale_price) is null THEN 0 ELSE sum(item.sale_price) end importe_ar_erp,rece.billing_date,rece.credit_date from prod_lake_oic_refined.order_to_cash otc inner join prod_lake_oic_refined.receivable rece on otc.id = rece.order_to_cash_id left join prod_lake_oic_refined.invoice invo on otc.id = invo.order_to_cash_id left join prod_lake_oic_refined.invoice invoice on otc.id = invoice.order_to_cash_id left join prod_lake_oic_refined.invoice_items item on invoice.id = item.id_invoice where rece.billing_date between cast('" + str(fecha_inicio) + " 00:00:00' as timestamp) and cast('" + str(fecha_fin) + " 00:00:00' as timestamp)  group by otc.minifactu_id ,otc.country,otc.operation,rece.registration_gym_student,otc.front_id,otc.origin_system ,rece.erp_clustered_receivable_id,rece.billing_date,rece.credit_date;")
        records = cursor.fetchall()
        ...
        for row in records:
            ...
            billing_date = str(row[9])
            credit_date = str(row[10])
            cur = connposgresql.cursor()
            query_sql_insert = 'insert into "DATALAKE".oic_otc (minifactu_id,country,operation,registration_gym_student,front_id,origin_system,erp_clustered_receivable_id,list_price,importe_ar_erp,billing_date,credit_date) ' \
                               "values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"

            cur.execute(query_sql_insert, (
            minifactu_id,country,operation,registration_gym_student,front_id,origin_system,erp_clustered_receivable_id,list_price,importe_ar_erp,billing_date,credit_date))
        connposgresql.commit()
        cursor.close()
        return jsonify({'status': 'success', 'message': 'Sincronizacion Front System Finalizada con exito!!!'}), 200
    except Exception as e:
